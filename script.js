const curtainLeft = document.getElementById('curtain-left');
    ...
    const weddingDate = new Date('December 21, 2023 00:00:00').getTime();
    ...
    if (timeRemaining <= 0) {
        ...
        countdownTimer.innerHTML = 'The wedding is here!';
        }
...
// Add the following code for Tinder-like swipe cards
    ...
    // Function to initialize family cards
function initFamilyCards() {
    var familyContainer = document.getElementById('familyCards');
    familyContainer.innerHTML = ''; // Clear existing cards

    var familiesData = [
        { image: 'f1.png', description: 'Family 1 Description' },
        { image: 'f2.png', description: 'Family 2 Description' },
    ...
    familiesData.forEach((family, index) => {
        ...
        card.innerHTML = `
            <img src="${family.image}" alt="Family Image">
...
// Initialize family cards on page load
initFamilyCards();

// Reset family cards on button click
document.getElementById('resetFamilyButton').addEventListener('click', function () {
    resetFamilyCards();
});

// Function to reset family cards
function resetFamilyCards() {
    const removedCards = document.querySelectorAll('.tinder--card.removed');
    removedCards.forEach((card) => {
        card.classList.remove('removed');
        card.style.transform = '';
    });
    setTimeout(() => initFamilyCards(), 300);// Re-initialize family cards
}